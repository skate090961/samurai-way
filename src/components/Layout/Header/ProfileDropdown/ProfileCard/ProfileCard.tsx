...
const ProfileCard: React.FC<ProfileCardPropsType> = ({ profile, setIsShowCard }) => {
  ...
  return (
    <div className={s.profile_card}>
      <div className={s.header}>
        <img
          src={profile.photos?.small ? profile.photos.small : defaultPhoto}
          alt={"user_photo"}
      ...
      <div className={s.view_profile_button}>
        <Link to={"/profile"} onClick={viewProfileHandler}>
          View profile
        </Link>
      </div>
      <div className={s.button_container}>
        <IoSettingsOutline size={25} />
        <Link to={"/settings"} onClick={settingsHandler}>
          Settings
        </Link>
      </div>
      <div className={s.button_container}>
