...
import { removeMessageAC, setDialogsAC, setMessagesAC, setTotalItemsCountAC, updateMessagesAC } from "./message-reducer"
...
export const getMessagesTC = (userId: number) => async (dispatch: Dispatch, getState: () => RootStateType) => {
  dispatch(setAppStatusAC("loading"))
  const { pageSize, currentPage } = getState().chatPage.messages
  try {
    const response = await dialogsAPI.getMessages(userId, currentPage, pageSize)
    if (!response.error) {
...
export const sendMessageTC =
  (senderId: number, message: string) => async (dispatch: AppDispatch, getState: () => RootStateType) => {
    dispatch(setAppStatusAC("loading"))
    try {
...
export const removeMessageTC = (messageId: string) => async (dispatch: Dispatch) => {
  dispatch(setAppStatusAC("loading"))
  try {
    const response = await dialogsAPI.removeMessage(messageId)
    if (response.resultCode === 0) {
